...
class Kohana_URL {
	 ...
	 * If a protocol is used, a fulle URL will be generated using the
	 ...
	 */
	public static function base($protocol = 'http', $index = FALSE)
	{
		$config_protocol = Kohana::$config->load('config.protocol');

		if(!is_null($config_protocol)){
		...
		$protocol = str_replace('httpss','https', $protocol);
		...
		if (is_string($protocol))
		{
			...
			else
			{
				...
				$domain = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : (isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : 'localhost');
			}
